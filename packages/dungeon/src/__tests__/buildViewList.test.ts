    ...
    for (const lateral of [0, 1, -1]) {
      if (depth < 3) {
        slots[`front:${lateral}:${depth}`] = {
          draws: [{ bank: 'b', frame: `front-${lateral}-${depth}`, destX: 0, destY: 0, blend: 'replace' }],
        };
      }
    }
    slots[`side:L:${depth}`] = { draws: [{ bank: 'b', frame: `side-L-${depth}`, destX: 0, destY: 0, blend: 'mask' }] };
    slots[`side:R:${depth}`] = { draws: [{ bank: 'b', frame: `side-R-${depth}`, destX: 0, destY: 0, blend: 'mask' }] };
  }
  return {
    schemaVersion: 1,
...
describe('buildViewList', () => {
  it('emits nothing when there are no walls anywhere', () => {
    ...
    const items = buildViewList(level, POSE, viewSpecFromSlotTable(makeSlots()), SEMANTICS, makeSlots());
  ...
  it('skips a cell that projects out of bounds without throwing', () => {
    ...
    const level = makeLevel({});
    expect(() => buildViewList(level, edgePose, viewSpecFromSlotTable(makeSlots()), SEMANTICS, makeSlots())).not.toThrow();
  });
  ...
  it('forwards every PieceDraw field from the slot entry untouched', () => {
    ...
    slots.slots['front:0:1'] = {
      draws: [{ bank: 'b', frame: 'x', destX: 16, destY: 5, mirrorX: true, blend: 'replace', priority: 0x64, origin: 'cite' }],
    };
    const items = buildViewList(level, POSE, viewSpecFromSlotTable(slots), SEMANTICS, slots);
    expect(items[0]).toMatchObject({
      bank: 'b', frame: 'x', destX: 16, destY: 5, mirrorX: true, blend: 'replace', priority: 0x64, origin: 'cite',
    });
  });
});
