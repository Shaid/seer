...
function loadRampPalette(name: string) {
  return parseRampPalette(
    JSON.parse(readFileSync(`${FIXTURES}${name}`, 'utf8')) as RampPaletteFile,
  );
}
...
describe('PieceBank.fromIndexedRGBA + ramp swap — real bcdfx assets', () => {
  ...
  const atlas = JSON.parse(
    readFileSync(`${FIXTURES}dungeon-bcdfx-indexed.json`, 'utf8'),
  ) as AtlasMeta;
  ...
  it('decodes the real indexed atlas against its own baked (ramp0) palette with zero mismatches', () => {
    ...
    expect(() =>
      PieceBank.fromIndexedRGBA(index.rgba, mask.rgba, index.width, index.height, atlas, ramp0),
    ).not.toThrow();
  });

  it('recovered indices round-trip byte-exact back to the source RGBA through the same (ramp0) palette', () => {
    const bank = PieceBank.fromIndexedRGBA(
      index.rgba,
      mask.rgba,
      index.width,
      index.height,
    ...
    for (let i = 0; i < bank.index.length; i++) {
      const o = i * 4;
      if (
        rebuilt[o] !== index.rgba[o] ||
        rebuilt[o + 1] !== index.rgba[o + 1] ||
        rebuilt[o + 2] !== index.rgba[o + 2]
      )
        mismatches++;
    }
  ...
  it('rejects being paired with a ramp that is not actually the atlas\'s baked ramp (a wrong-ramp pairing does not silently "work")', () => {
    ...
    expect(() =>
      PieceBank.fromIndexedRGBA(index.rgba, mask.rgba, index.width, index.height, atlas, ramp3),
    ).toThrow(/isn't in basePalette/);
  });
  ...
  it('the SAME recovered index buffer renders to different RGBA colors under ramp0 vs ramp3 (the actual M4 bug fix, not just "didn\'t crash")', () => {
    ...
    for (let i = 0; i < bank.index.length; i++) {
      const o = i * 4;
      if (
        underRamp0[o] !== underRamp3[o] ||
        underRamp0[o + 1] !== underRamp3[o + 1] ||
        underRamp0[o + 2] !== underRamp3[o + 2]
      ) {
        differingPixels++;
      }
    }
  ...
  it('exposes real bank frames from the indexed atlas (same frame-lookup API as fromRGBA)', () => {
    ...
    expect(bank.frame(firstFrame.name)).toEqual({
      x: firstFrame.x,
      y: firstFrame.y,
      w: firstFrame.w,
      h: firstFrame.h,
