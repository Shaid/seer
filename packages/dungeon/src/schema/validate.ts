...
import type { DungeonLevelFile, CellSpace, WallStorage, LevelUnit, Dir4 } from './level.js';
import type {
  SlotTableFile,
  PieceBankRef,
  PieceDraw,
...
import type { SemanticsFile, WallMeaning, FeatureMeaning } from './semantics.js';
import { BINDING_ACTIONS, type BindingsFile, type BindingAction } from './bindings.js';
...
function assertNumber(v: unknown, context: string, field: string): number {
  if (typeof v !== 'number' || !Number.isFinite(v))
    fail(context, `"${field}" must be a finite number, got ${typeof v}`);
  return v;
}
...
function assertObject(v: unknown, context: string, field: string): Record<string, unknown> {
  if (!isPlainObject(v))
    fail(context, `"${field}" must be an object, got ${v === null ? 'null' : typeof v}`);
  return v;
}

function assertDir4(v: unknown, context: string, field: string): Dir4 {
  if (v !== 0 && v !== 1 && v !== 2 && v !== 3)
    fail(context, `"${field}" must be a Dir4 (0-3), got ${JSON.stringify(v)}`);
  return v;
}
...
function validateCellSpace(v: unknown, context: string): CellSpace {
  ...
  if (o.kind === 'flat') {
    return {
      kind: 'flat',
      width: assertNumber(o.width, context, 'cellSpace.width'),
      height: assertNumber(o.height, context, 'cellSpace.height'),
    };
  }
  if (o.kind === 'regions') {
    return {
...
function validateWallStorage(v: unknown, context: string): WallStorage {
  ...
  if (o.kind === 'bitflags') {
    ...
    if (bits.length !== 4)
      fail(context, `wallStorage.bits must have exactly 4 entries, got ${bits.length}`);
    return {
      kind: 'bitflags',
      plane: assertString(o.plane, context, 'wallStorage.plane'),
      bits: bits.map((b, i) => assertNumber(b, context, `wallStorage.bits[${i}]`)) as [
        number,
        number,
        number,
        number,
  ...
  if (o.kind === 'shared-edge') {
    ...
    if (planes.length !== 2)
      fail(context, `wallStorage.planes must have exactly 2 entries, got ${planes.length}`);
    if (planeDirs.length !== 2)
      fail(context, `wallStorage.planeDirs must have exactly 2 entries, got ${planeDirs.length}`);
    return {
      kind: 'shared-edge',
      planes: [
        assertString(planes[0], context, 'wallStorage.planes[0]'),
        assertString(planes[1], context, 'wallStorage.planes[1]'),
      ],
      planeDirs: [
        assertDir4(planeDirs[0], context, 'wallStorage.planeDirs[0]'),
        assertDir4(planeDirs[1], context, 'wallStorage.planeDirs[1]'),
      ],
  ...
  if (o.kind === 'per-cell-4') {
    const planes = assertArray(o.planes, context, 'wallStorage.planes');
    if (planes.length !== 4)
      fail(context, `wallStorage.planes must have exactly 4 entries, got ${planes.length}`);
    return {
      kind: 'per-cell-4',
      planes: planes.map((p, i) => assertString(p, context, `wallStorage.planes[${i}]`)) as [
        string,
        string,
        string,
        string,
  ...
  fail(
    context,
    `wallStorage.kind must be "bitflags", "shared-edge" or "per-cell-4", got ${JSON.stringify(o.kind)}`,
  );
}
...
function validateLevelUnit(v: unknown, context: string): LevelUnit {
  ...
  return {
    ...
    planes,
    sublevelPlane:
      o.sublevelPlane === undefined
        ? undefined
        : assertString(o.sublevelPlane, context, 'unit.sublevelPlane'),
    sublevels:
      o.sublevels === undefined
        ? undefined
        : assertArray(o.sublevels, context, 'unit.sublevels').map((s, i) => {
            ...
            return {
              ...
              label:
                so.label === undefined
                  ? undefined
                  : assertString(so.label, context, `unit.sublevels[${i}].label`),
              tileset:
                so.tileset === undefined
                  ? undefined
                  : assertString(so.tileset, context, `unit.sublevels[${i}].tileset`),
              paletteRamp:
                so.paletteRamp === undefined
                  ? undefined
                  : assertNumber(so.paletteRamp, context, `unit.sublevels[${i}].paletteRamp`),
            };
          }),
    tileset: o.tileset === undefined ? undefined : assertString(o.tileset, context, 'unit.tileset'),
    paletteRamp:
      o.paletteRamp === undefined
        ? undefined
        : assertNumber(o.paletteRamp, context, 'unit.paletteRamp'),
  };
}
...
export function validateDungeonLevelFile(v: unknown): DungeonLevelFile {
  ...
  const units = assertArray(o.units, context, 'units').map((u, i) =>
    validateLevelUnit(u, `${context}.units[${i}]`),
  );
  ...
  if (o.entities !== undefined) {
    ...
    for (const [key, rec] of Object.entries(entitiesObj)) {
      ...
      entities[key] = {
        ...
        gfx:
          ro.gfx === undefined ? undefined : assertNumber(ro.gfx, context, `entities.${key}.gfx`),
        wallMask:
          ro.wallMask === undefined
            ? undefined
            : assertNumber(ro.wallMask, context, `entities.${key}.wallMask`),
        slotNibble:
          ro.slotNibble === undefined
            ? undefined
            : assertNumber(ro.slotNibble, context, `entities.${key}.slotNibble`),
        chainNext:
          ro.chainNext === undefined
            ? undefined
            : assertNumber(ro.chainNext, context, `entities.${key}.chainNext`),
        flags:
          ro.flags === undefined
            ? undefined
            : assertNumber(ro.flags, context, `entities.${key}.flags`),
        raw:
          ro.raw === undefined
            ? undefined
            : assertArray(ro.raw, context, `entities.${key}.raw`).map((n, i) =>
                assertNumber(n, context, `entities.${key}.raw[${i}]`),
              ),
      };
    }
  }
  ...
  return {
    ...
    entities,
    entityHandlePlane:
      o.entityHandlePlane === undefined
        ? undefined
        : assertString(o.entityHandlePlane, context, 'entityHandlePlane'),
    provenance:
      o.provenance === undefined
        ? undefined
        : (assertObject(o.provenance, context, 'provenance') as Record<string, string>),
  };
}
...
function validatePieceBankRef(v: unknown, context: string): PieceBankRef {
  ...
  return {
    ...
    indexed:
      o.indexed === undefined ? undefined : assertBoolean(o.indexed, context, 'bank.indexed'),
...
function validateAnimRef(v: unknown, context: string): AnimRef {
  ...
  const frames = assertArray(o.frames, context, 'draw.frame.frames').map((f, i) =>
    assertString(f, context, `draw.frame.frames[${i}]`),
  );
  const phase =
    o.phase === undefined ? undefined : assertString(o.phase, context, 'draw.frame.phase');
  ...
  return {
    ...
    periodTicks:
      o.periodTicks === undefined
        ? undefined
        : assertNumber(o.periodTicks, context, 'draw.frame.periodTicks'),
    phase: phase as AnimRef['phase'],
    phaseTicks:
      o.phaseTicks === undefined
        ? undefined
        : assertNumber(o.phaseTicks, context, 'draw.frame.phaseTicks'),
  };
}
...
function validatePieceDraw(v: unknown, context: string): PieceDraw {
  ...
  if (typeof blend !== 'string' || !BLEND_MODES.includes(blend as BlendMode)) {
    fail(
      context,
      `draw.blend must be one of ${BLEND_MODES.join(', ')}, got ${JSON.stringify(blend)}`,
    );
  }
  return {
    ...
    mirrorX:
      o.mirrorX === undefined ? undefined : assertBoolean(o.mirrorX, context, 'draw.mirrorX'),
    blend: blend as BlendMode,
    maskSource:
      o.maskSource === undefined
        ? undefined
        : (assertString(o.maskSource, context, 'draw.maskSource') as 'plane' | 'index'),
    maskIndex:
      o.maskIndex === undefined ? undefined : assertNumber(o.maskIndex, context, 'draw.maskIndex'),
    priority:
      o.priority === undefined ? undefined : assertNumber(o.priority, context, 'draw.priority'),
    hotspot:
      o.hotspot === undefined
        ? undefined
        : {
            code: assertNumber(
              assertObject(o.hotspot, context, 'draw.hotspot').code,
              context,
              'draw.hotspot.code',
            ),
          },
...
function validateXYPair(v: unknown, context: string, field: string): [number, number] {
  ...
  if (a.length !== 2)
    fail(context, `"${field}" must be a 2-element [x, y] array, got ${a.length} elements`);
  return [assertNumber(a[0], context, `${field}[0]`), assertNumber(a[1], context, `${field}[1]`)];
}

function validateXYPairRecord(
  v: unknown,
  context: string,
  field: string,
  ...
  for (const [key, pair] of Object.entries(o))
    out[key] = validateXYPair(pair, context, `${field}.${key}`);
  return out;
}
...
function validateFloorItemPlacement(v: unknown, context: string): FloorItemPlacement {
  ...
  return {
    ...
    gfxToGroup: assertArray(o.gfxToGroup, context, 'floorItem.gfxToGroup').map((n, i) =>
      assertNumber(n, context, `floorItem.gfxToGroup[${i}]`),
    ),
...
export function validateSlotTableFile(v: unknown): SlotTableFile {
  ...
  const banks = assertArray(o.banks, context, 'banks').map((b, i) =>
    validatePieceBankRef(b, `${context}.banks[${i}]`),
  );
  const lateralOffsets = assertArray(o.lateralOffsets, context, 'lateralOffsets').map((n, i) =>
    assertNumber(n, context, `lateralOffsets[${i}]`),
  );
  ...
  const ordering =
    o.ordering === undefined ? undefined : assertString(o.ordering, context, 'ordering');
  if (ordering !== undefined && ordering !== 'array' && ordering !== 'painter-back-to-front') {
    fail(
      context,
      `ordering must be "array" or "painter-back-to-front", got ${JSON.stringify(ordering)}`,
    );
  }
  ...
  return {
    schemaVersion: 1,
    surface: {
      width: assertNumber(surface.width, context, 'surface.width'),
      height: assertNumber(surface.height, context, 'surface.height'),
    },
    viewport: {
    ...
    slots,
    staticSlots:
      o.staticSlots === undefined
        ? undefined
        : assertArray(o.staticSlots, context, 'staticSlots').map((s, i) => {
    ...
    floorItem:
      o.floorItem === undefined ? undefined : validateFloorItemPlacement(o.floorItem, context),
  };
}
...
function validateWallMeaning(v: unknown, context: string): WallMeaning {
  ...
  return {
    ...
    discoveredPieceKind:
      o.discoveredPieceKind === undefined
        ? undefined
...
function validateFeatureMeaning(v: unknown, context: string): FeatureMeaning {
  ...
  return {
    ...
    pieceKind:
      o.pieceKind === null ? null : assertString(o.pieceKind, context, 'feature.pieceKind'),
    orientationGated:
      o.orientationGated === undefined
        ? undefined
        : assertBoolean(o.orientationGated, context, 'feature.orientationGated'),
    blocksMovement:
      o.blocksMovement === undefined
        ? undefined
        : assertBoolean(o.blocksMovement, context, 'feature.blocksMovement'),
...
export function validateSemanticsFile(v: unknown): SemanticsFile {
  ...
  if (typeof o.confidence !== 'string' || !CONFIDENCE_LEVELS.includes(o.confidence)) {
    fail(
      context,
      `confidence must be one of ${CONFIDENCE_LEVELS.join(', ')}, got ${JSON.stringify(o.confidence)}`,
    );
  }

  const wallsObj = assertObject(o.walls, context, 'walls');
  const walls: Record<string, WallMeaning> = {};
  for (const [key, w] of Object.entries(wallsObj))
    walls[key] = validateWallMeaning(w, `${context}.walls.${key}`);

  const featuresObj = assertObject(o.features, context, 'features');
  const features: Record<string, FeatureMeaning> = {};
  for (const [key, f] of Object.entries(featuresObj))
    features[key] = validateFeatureMeaning(f, `${context}.features.${key}`);
  ...
  if (o.facingMap !== undefined) {
    ...
    facingMap = arr.map((d, i) => assertDir4(d, context, `facingMap[${i}]`)) as [
      Dir4,
      Dir4,
      Dir4,
      Dir4,
  ...
  return {
    ...
    facingMap,
    yAxisDown:
      o.yAxisDown === undefined ? undefined : assertBoolean(o.yAxisDown, context, 'yAxisDown'),
  };
}
...
export function validateBindingsFile(v: unknown): BindingsFile {
  ...
  const extra = Object.keys(bindingsObj).filter(
    (k) => !BINDING_ACTIONS.includes(k as BindingAction),
  );
  if (extra.length > 0) {
    fail(
      context,
      `bindings has unknown action(s) ${extra.join(', ')} — expected only ${BINDING_ACTIONS.join(', ')}`,
    );
  }
